...
object FactorialMain extends App {
  ...
  val programZIO: ZIO[ConsoleIO with Calculator, Throwable, Unit] = for {
    _ <- consoleZIO.putStr("Please enter a number: ")
    input <- consoleZIO.getStr
    _ <- Validation
      .validateInt(input)
      .fold[ZIO[ConsoleIO with Calculator, Throwable, Unit]](
      consoleZIO.putStr("Invalid number"))(number =>
          ZIO
            .accessM[Calculator](_.calculator.factorial(number)) >>= (factorial =>
            consoleZIO.putStr(s"Factorial of $number is $factorial")))
  } yield ()
