...
object StateEx {
  ...
  class UserService(cache: Cache, repository: Repository) {
    ...
    def findById(id: Int): String \/ User = {
      ...
      mu.cata(_.right, { // cata is a method on option whihc takes two lambdas first one of which will be invoked if option has some value otherwise the second one
