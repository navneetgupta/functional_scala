...
trait Stream[F[_]] {
  ...
  def inputOutputStream(in: File, out: File): Resource[F, (FileInputStream, FileOutputStream)]
}
...
object FileContentTransferEx {
  ...
  def inputOutputStream[F[_]: Stream](in: File, out: File): Resource[F, (FileInputStream, FileOutputStream)] =
    Stream[F].inputOutputStream(in, out)

  def copy[F[_]: Transfer](source: File, dest: File): F[Long] = Transfer[F].copy(source, dest)

  def transfer[F[_]: Transfer](input: InputStream, output: OutputStream): F[Long] = Transfer[F].transfer(input, output)
  ...
  def main[F[_]: Stream : Transfer : Monad : Console](source: File, dest: File): F[Unit] = {
    for {
      count <- FileContentTransferEx.copy(source, dest)
      _     <- putStrLn(s"$count bytes copied from ${source.getPath} to ${dest.getPath}")
    } yield ()
  }
...
object ExampleApp extends IOApp {
  ...
  implicit  val TransferIO = new Transfer[IO] {
    def copy(source: File, dest: File): IO[Long] =
      FileContentTransferEx.inputOutputStream(source, dest).use { case (in, out) =>
        transfer(in, out)
      }
  ...
  override def run(args: List[String]): IO[ExitCode] =
    for {
      ...
      _      <- FileContentTransferEx.main[IO](orig, dest)
